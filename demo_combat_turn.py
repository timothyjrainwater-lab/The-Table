...
def run_demo(with_tts: bool = False) -> dict:
    ...
    result_line("Damage", f"{weapon.damage_dice}+{weapon.damage_bonus} {weapon.damage_type}")
    ...
    for event in turn_result.events:
        ...
        if etype == "turn_start":
            ...
            result_line("Damage Roll", f"{dice}+{bonus} = [{rolls_str}]+{bonus} = {total} {dtype}")
    ...
    template_pass = turn_ms < 500  # A8 criterion: template <500ms p95
    if template_pass:
        ok(f"Template path: {turn_ms:.1f}ms (target: <500ms p95)")
    else:
    ...
    a8_criteria = [
        ...
        ("Performance: template <500ms p95", audit.get("template_performance", False)),
