...
**Last triaged:** 2026-03-01 (session 28) — Batch AM ACCEPTED (PM verdict). FINDING-ENGINE-FREE-HAND-SETTER-002 → CLOSED (builder). FINDING-ENGINE-CONDITION-NORMALIZE-DEAD-CODE-001 → CLOSED (builder). FINDING-ENGINE-CL-SCALED-DAMAGE-MISSING-001 → CLOSED (PM). FINDING-ENGINE-CL-SCALED-DURATION-MISSING-001 → CLOSED (PM). FINDING-ENGINE-CONDITIONS-LEGACY-FORMAT-001 → CLOSED (PM, missed in initial close-out). FINDING-PROCESS-BUILDER-PRE-ACCEPTED-RECURRING-001 → escalated LOW→MEDIUM (3rd occurrence). New LOW: FINDING-ENGINE-MAGIC-MISSILE-CL-SCALE-001 + FINDING-ENGINE-TWF-FREE-HANDS-001 + FINDING-ENGINE-FULL-SUITE-FAILURE-DELTA-001. ML-002 filed: FINDING-ENGINE-CAUSE-FEAR-DURATION-SPEC-ERROR-001 (MEDIUM) + FINDING-ENGINE-CL-CONDITION-DEFAULT-TRAP-001 + FINDING-ENGINE-LONGBOW-FREE-HANDS-CATALOG-001 + FINDING-ENGINE-NORMALIZE-CONDITION-DICT-DEAD-001 (all LOW). Sweep counter: 3/5 — AK+AL+AM; sweep due after AN+AO.
...
- **IN FLIGHT:** Finding has a WO dispatched — must carry: WO ID + dispatch date + expiry (dispatch date + 2 batches). If no debrief filed before expiry, finding reverts to OPEN for re-triage. Do not re-dispatch without re-triage.
...
| FINDING-ENGINE-CAUSE-FEAR-DURATION-SPEC-ERROR-001 | `cause_fear` (`spell_definitions.py`) has `duration_rounds_per_cl=1` citing PHB p.208 — but that cite is **wrong**. PHB p.208 `cause_fear` (1st level): duration = "1d4 rounds." `1 round/level` belongs to *fear* (4th level, PHB p.229). RAW drift introduced by CDU WO spec; no gate test catches it (CDU tests verify the scaling mechanism, not the specific spell's PHB duration value). Fix: remove `duration_rounds_per_cl`; set static `duration_rounds=2` (conservative 1d4 midpoint) with NOTE, or add a flat-random draw hook for spells requiring dice duration. | Batch AM ML-002 (Chisel, 2026-03-01) | OPEN — WO needed; cause_fear currently scales over-long at high CL |
...
### Engine — Spellcasting Audit (WO-AUDIT-SPELL-012, 2026-03-01)

| ID | Summary | Filed | Status |
|----|---------|-------|--------|
| FINDING-AUDIT-SPELL-012-ILLUSION-DC-CONSUME-001 | `EF.ILLUSION_DC_BONUS` set for gnomes at `races.py:318`, defined in `entity_fields.py:272`, **never consumed** in `_create_caster_stats()` or any spell DC path. `CasterStats.get_spell_dc()` = spell_dc_base + spell_level + spell_focus_bonus — gnome illusion spell DC missing PHB p.16 +1. Different consumer from existing DEFERRED FINDING-ENGINE-RACIAL-ILLUSION-WRITE-ONLY-001 (which groups illusion_dc_bonus with "no illusion subsystem"). This bonus needs no subsystem — just a 1-line read in `_create_caster_stats()` for casters with illusion school spells. | WO-AUDIT-SPELL-012 (2026-03-01) | **CLOSED** — WO-ENGINE-ILLUSION-DC-WIRE-001 ACCEPTED (dbceb91, 2026-03-01). 2-line insertion at play_loop.py:1064-1066. Gnome +1 illusion DC now applied via `_spell_focus_bonus`. |
| FINDING-AUDIT-SPELL-012-DRUID-SPONTANEOUS-SUMMON-001 | Druid spontaneous summon nature's ally (PHB p.35) not implemented **and has no NOT STARTED row in ENGINE_COVERAGE_MAP.md §8**. Coverage map covers only cleric cure/inflict spontaneous. Spontaneous casting code at play_loop.py handles only `spontaneous_cure` and `spontaneous_inflict` flags. Two gaps: (1) missing engine mechanic, (2) missing coverage map row. | WO-AUDIT-SPELL-012 (2026-03-01) | **CLOSED** — WO-ENGINE-DRUID-SPONTANEOUS-SUMMON-001 ACCEPTED (dbceb91, 2026-03-01). elif block wired; same-level SNA redirect live. "Or lower" clause → FINDING-ENGINE-DRUID-SUMMON-LOWER-LEVEL-001 (LOW, DEFERRED). |
...
### Governance — Process Integrity (filed 2026-03-01)

| ID | Summary | Filed | Status |
|----|---------|-------|--------|
| FINDING-GOV-RESEARCH-REGISTRY-001 | No master registry of research artifacts vs. their sweep status. `docs/OSS_RESEARCH_SPRINT_001.md`, `docs/design/STRAT-AIDM-JUDGMENT-LAYER-001.md`, and ~12 other research docs in `docs/design/` and `docs/specs/` have no tracked disposition (swept / not swept / backlog conversion complete). Without a registry, sweeps can be declared complete without proving every file was touched. `docs/ops/REGISTER-RESEARCH-ARTIFACTS.md` created as skeleton — needs one-time Anvil sweep to validate dispositions. | Session 27 — governance gap analysis (2026-03-01) | **CLOSED** — WO-AUDIT-GOV-REGISTRY-001 ACCEPTED (2026-03-01). 19 artifacts swept (13 SWEPT+LEDGERED, 6 PARTIAL — size-limited). Registry update table in debrief. Debrief: pm_inbox/reviewed/DEBRIEF_WO-AUDIT-GOV-REGISTRY-001.md |
| FINDING-GOV-SELF-REVIEW-VERDICT-001 | PM writes Acceptance Notes at dispatch, PM verifies them at verdict. No independent reviewer for any verdict. For HIGH-severity WOs (engine resolvers, data ingestion, judgment layer), self-review is insufficient. Pattern: 17 write-only EF fields, save double-count, SR shadow path — all passed PM verdict initially. Options: (a) Anvil spot-checks HIGH verdicts, (b) PM explicitly flags "self-review only" in verdict record so risk is visible. Verdict Review Class (SELF-REVIEW / SELF-REVIEW + ANVIL SPOT-CHECK / ANVIL PRIMARY) added to CLAUDE.md and WO_DISPATCH_TEMPLATE.md per Addendum v2. Escalation trigger defined — Thunder does not need to decide, PM makes call per classification rubric. | Session 27 — governance gap analysis (2026-03-01) | ACTIONED — Verdict Review Class wired into CLAUDE.md + WO_DISPATCH_TEMPLATE.md. Monitor: verify PM applies classification on next 3 verdicts. |
| FINDING-PROCESS-BUILDER-PRE-ACCEPTED-RECURRING-001 | Builder pre-populates ACCEPTED verdict in PM artifacts (PM_BRIEFING_CURRENT.md, BACKLOG_OPEN.md) before PM issues verdict. 3rd occurrence (AG + AH + AM). Correct status is FILED. Root cause: builder modifying PM-owned artifacts during debrief process — BACKLOG_OPEN.md header, sweep counter, and finding statuses all altered pre-verdict. Escalated LOW→MEDIUM on 3rd occurrence. | Session 28 (PM verdict AM review) | OPEN — requires dispatch rule enforcement; builder must not touch PM artifacts |
| FINDING-GOV-PAST-RESEARCH-NO-LEDGER-001 | "Phase 2 OSS research COMPLETE — 3 passes" declared without per-section completeness ledger (RUD-001/§6 format). `docs/OSS_RESEARCH_SPRINT_001.md` has no FILE RESULT entry proving every section was dispositioned. Evidence of gap: `katekorsaro/dnd3.5e-srd` in the sprint doc (Tier 3 — reference) has no backlog entry and no WO. One-time retrospective ledger pass required. Not a re-research — a disposition audit against existing BACKLOG_OPEN entries. | Session 27 — governance gap analysis (2026-03-01) | **CLOSED** — WO-AUDIT-GOV-REGISTRY-001 (2026-03-01). katekorsaro confirmed REFERENCE-ONLY per STRATEGY-OSS-INTEGRATION-001 (Decision=REFERENCE, WO Candidate=None, Phase=Ongoing). Sprint doc Tier 3 fully dispositioned. All other items already tracked.
...
| ID | Summary | Filed | Status |
|----|---------|-------|--------|
| FINDING-AUDIT-GOV-IDEMPOTENCY-PATTERN-001 | `docs/design/STRATEGY-OSS-INTEGRATION-001.md` §"Entity Lifecycle / State Machines (idempotency)" specifies the Evennia Tags idempotency pattern as a coding standard: "apply as standard pattern in all new resolvers" — prevents double-trigger on retry via `if "event_name" not in entity.triggered_events: fire_event(); entity.triggered_events.add("event_name")`. This pattern is NOT documented in `BUILDER_FIELD_MANUAL.md`. Risk: future builders implement events without idempotency guards. Low urgency — only blocks live-play session robustness, not correctness of any current gate test. | WO-AUDIT-GOV-REGISTRY-001 (2026-03-01) | OPEN — add to BUILDER_FIELD_MANUAL as standard coding pattern |
| FINDING-AUDIT-GOV-CREATIVE-ADVERSARIAL-PACK-001 | `docs/design/REDTEAM-CREATIVE-ADVERSARIAL-001.md` references "PACK-CREATIVE-ADVERSARIAL-INTENTS-001 Phase 2" expansion — a planned pack of creative adversarial intent test cases beyond the current HOOLIGAN suite. No backlog entry exists for this Phase 2 expansion. Low urgency — Phase 2 gated, depends on judgment layer Shadow phase. | WO-AUDIT-GOV-REGISTRY-001 (2026-03-01) | DEFERRED — Phase 2 gate (judgment layer Shadow phase must exit first) |
| FINDING-AUDIT-GOV-PARTIAL-READS-001 | 6 artifacts in the WO-AUDIT-GOV-REGISTRY-001 sweep could only be read via preview (first 2KB) due to file size. Full section-by-section audit of these files deferred: (1) `docs/design/LEDGER-CHEEVO-CANARY-EXPANSION-001.md` — 96.6KB, full TC routing table not visible; (2) `docs/PHB_35_MECHANICS_INVENTORY_PART1.md` — 76.8KB; (3) `docs/PHB_35_MECHANICS_INVENTORY_PART2.md` — 57KB; (4) `docs/PHB_35_MECHANICS_INVENTORY_PART3.md` — 81.5KB; (5) `docs/doctrine/ROUNDTABLE_INTEGRATION_DESIGN_001.md` — 52.7KB; (6) `docs/MASTER_BUILD_PLAN_001.md` — 487 lines, only 100 read (Phases 2-7 not reviewed). PHB parts and LEDGER are highest priority for dedicated re-read. | WO-AUDIT-GOV-REGISTRY-001 (2026-03-01) | DEFERRED — low urgency (no critical items expected in partial files; PHB contents tracked in ENGINE_COVERAGE_MAP; LEDGER routing covered by CANARY-GAP-TAXONOMY-001) |
| FINDING-DATA-SRD-PSIONIC-SKILLS-001 | `aidm/data/srd_skills.json` includes 3 psionic skills (autohypnosis, psicraft, use_psionic_device) and control_shape — present in zellfaze-zz/dnd-generator source (CC0). Not filtered. WO-JUDGMENT-VALIDATOR-001 may need to filter if PHB-core-only skill coverage is required (`magic` and `psi` boolean flags are in source but NOT exported to srd_skills.json). | WO-DATA-SRD-EXTRACT-001 (2026-03-01) | DEFERRED — Phase 2 gate (WO-JUDGMENT-VALIDATOR-001 can filter at consume time; no immediate correctness impact) |
| FINDING-DATA-SRD-SPEAK-LANGUAGE-ABILITY-001 | `srd_skills.json` exports `ability: ""` for Speak Language (source has `key: ""` — no ability check; PHB p.82 correct). WO-JUDGMENT-VALIDATOR-001 must handle empty ability strings gracefully. | WO-DATA-SRD-EXTRACT-001 (2026-03-01) | DEFERRED — Phase 2 gate (WO-JUDGMENT-VALIDATOR-001 will handle at consume time; empty string is intentionally correct per PHB) |
| FINDING-ENGINE-DRUID-SUMMON-LOWER-LEVEL-001 | PHB p.35: druid may "lose" any prepared spell to cast summon nature's ally of the **same level OR LOWER**. WO-ENGINE-DRUID-SPONTANEOUS-SUMMON-001 implements same-level redirect only (level N slot → SNA level N). Lower-level selection ("or lower" clause) requires caller to specify target SNA level explicitly and slot validation logic to permit upward-level sacrifice. | Batch AK WO2 debrief (2026-03-01) | DEFERRED — same-level redirect is the common case; lower-level requires UX/intent-model design (trust-caller pattern appropriate; file Batch AM or later) |
|----|---------|-------|--------|
...
| FINDING-ENGINE-FULL-SUITE-FAILURE-DELTA-001 | Full-suite pre-existing failures grew from 161 (AL baseline) to 219 (AM run) — delta of +58. Builder reports additional failures are infrastructure-dependent tests (posture_audit, visual_qa_002, ws_bridge) not caused by AM changes. Engine-only test baseline remains ~161. Until a dedicated regression agent normalizes the count, full-suite failure count is unreliable as a signal. | Batch AM debrief (801875d, 2026-03-01) | OPEN — investigate +58 delta; confirm builder's infrastructure-only claim before next engine batch |
| FINDING-ENGINE-CL-CONDITION-DEFAULT-TRAP-001 | `_create_condition_stp()` in `play_loop.py` has `caster_level: int = 0` default. If any call site omits `caster_level`, `effective_duration_rounds(0) = 0 × duration_rounds_per_cl = 0` → condition duration of 0 → permanent condition (None expiry) instead of a loud failure. Builder updated 2 known call sites for CDU WO; a missed 3rd site would be silent. Verify all call sites pass explicit CL; consider changing default to `None` with a ValueError guard. | Batch AM ML-002 (Chisel, 2026-03-01) | OPEN — grep all `_create_condition_stp` call sites; confirm all pass explicit caster_level |
| FINDING-ENGINE-LONGBOW-FREE-HANDS-CATALOG-001 | **CONFIRMED (2026-03-02 grep):** `equipment_catalog.json` stores `longbow` as `weapon_type: "ranged"` — not `"two-handed"`. FHS setter checks `weapon_type == "two-handed"` only → longbow archers get FREE_HANDS=1 (wrong; both hands occupied per PHB Table 7-5 p.116). FHS-006 gate test asserts FREE_HANDS=1 and passes, but is testing a catalog gap not RAW correctness. Fix requires: (1) add `weapon_type: "ranged_two_handed"` (or `grip_hands: 2` field) to longbow/shortbow/crossbow entries, (2) update FHS setter to check both types, (3) update FHS-006 expectation to FREE_HANDS=0. Affects any Deflect Arrows eligibility check for bow users. | Batch AM ML-002 (Chisel, 2026-03-02) | OPEN — catalog + setter + gate test fix; file WO |
| FINDING-ENGINE-NORMALIZE-CONDITION-DICT-DEAD-001 | `_normalize_condition_dict()` in `conditions.py` may now be fully unreachable. CLF WO (Batch AM WO4) added a "test-only" docstring comment. But if CF-007/CF-008 gate tests (Batch AI) call it directly, it retains one live test caller. Confirm: (a) no live production path calls it, (b) CF-007/CF-008 still call it directly, (c) if both confirmed, it is a deletion candidate with 2 gate tests to update. | Batch AM ML-002 (Chisel, 2026-03-01) | OPEN — deletion candidate; verify CF-007/CF-008 call pattern before filing cleanup WO |
...
| FINDING-ENGINE-DRUID-SUMMON-LOWER-LEVEL-001 | PHB p.35: "...summon nature's ally spell of the same level OR LOWER." Current implementation (WO-ENGINE-DRUID-SPONTANEOUS-SUMMON-001) only handles same-level redirect. The "or lower" clause (losing a level-N slot to cast SNA < N) requires the caller to specify target SNA level explicitly. Not implemented; trust-caller for now. Future WO must accept a `spontaneous_summon_level` parameter on SpellCastIntent (or separate intent field) and map it to the right SNA entry. | WO-ENGINE-DRUID-SPONTANEOUS-SUMMON-001 (2026-03-01) | DEFERRED — trust-caller pattern; lower-level selection is caller-side concern (same as cleric cure pat tern) |
| FINDING-ENGINE-MAGIC-MISSILE-CL-SCALE-001 | Magic missile CL scaling (PHB p.251): 1 missile at CL1, +1 per 2 CLs (max 5 missiles at CL9). Pattern differs from fireball/lightning_bolt — it's multi-missile count (not per-missile dice scaling). `effective_damage_dice()` mechanism does not apply cleanly. Requires separate missile-count resolver: `missiles = min(5, (caster_level + 1) // 2)`. CONSUME_DEFERRED — magic missile not in active session scope. | WO-ENGINE-CL-DAMAGE-SCALE-001 debrief (801875d, 2026-03-01) | DEFERRED — multi-missile count pattern; dispatch as standalone WO when magic missile enters play |
| FINDING-ENGINE-TWF-FREE-HANDS-001 | Two-weapon fighting (PHB p.160): both hands occupied → FREE_HANDS should be 0, blocking Deflect Arrows. Current FHS setter only checks primary weapon grip (two-handed vs one-handed). TWF with two light weapons each one-handed → both hands full → FREE_HANDS=0 not set. No TWF chargen mechanic exists yet; deferring until TWF is implemented. | WO-ENGINE-FREE-HAND-SETTER-001 debrief (801875d, 2026-03-01) | DEFERRED — TWF not in engine; revisit when TWF chargen dispatched |
| FINDING-ENGINE-FREE-HAND-SETTER-002 | `EF.FREE_HANDS` has no chargen/equip setter. Default of 1 works for bare-handed / one-handed; two-handed grip, TWF, or certain conditions should set it to 0. Setter deferred to equip WO. | Batch AI WO3 CONSUME_DEFERRED | **CLOSED** — WO-ENGINE-FREE-HAND-SETTER-001 ACCEPTED (801875d, 2026-03-01). Inventory scan fallback handles override path. FHS-001..008. |
...
| FINDING-ENGINE-CONDITION-NORMALIZE-DEAD-CODE-001 | `_normalize_condition_dict()` in `conditions.py` is production-dead — only CF-007/CF-008 gate tests call it directly. Live code path does not use it. Intentional for test verifiability but undocumented. Add comment: "# test-verifiability only — not called from live code." | Batch AI ML-002 (Chisel) | **CLOSED** — WO-ENGINE-CONDITIONS-LEGACY-FIX-001 (801875d, 2026-03-01). NOTE added to docstring. CLF-001..008. |
...
| FINDING-FAGU-RSAWC-DEAD-CODE-001 | resolve_single_attack_with_critical() retained unused | Batch U | **CLOSED** — Batch AL WO2 FAR (d2351e3). Function deleted (263 lines). 0 call sites confirmed. |
...
| FINDING-ENGINE-CL-SCALED-DAMAGE-MISSING-001 | No per-CL damage dice scaling in spell resolution — spells like fireball don't scale damage with CL | CL2 WO | **CLOSED** — WO-ENGINE-CL-DAMAGE-SCALE-001 ACCEPTED (801875d, 2026-03-01). CDS-001..008. |
| FINDING-ENGINE-CL-SCALED-DURATION-MISSING-001 | No per-CL duration scaling in spell resolution — buff/debuff durations don't scale with CL | CL2 WO | **CLOSED** — WO-ENGINE-CL-DURATION-SCALE-001 ACCEPTED (801875d, 2026-03-01). CDU-001..008. |
...
| FINDING-AUDIT-SPELL-012-SR-WORLD-STATE-GATE-001 | `spell_resolver.resolve_spell()` SR check is gated on `if world_state is not None` at the call to `check_spell_resistance()`. If world_state=None (direct test call), a target with `SR > 0` is silently unblocked — SR bypassed entirely. All production paths (play_loop) pass world_state, so no runtime bug. But test authors calling resolve_spell() directly will get SR-skipped results. Fix: extract SR roll before world_state check; only the Spell Penetration feats need world_state. | WO-AUDIT-SPELL-012 (2026-03-01) | **CLOSED** — Batch AL WO4 SRG (d2351e3). Explicit ValueError guard: if world_state=None and target SR>0, raise ValueError. Silent bypass now impossible; production path unaffected. |
| FINDING-AUDIT-SPELL-012-AOO-CONC-BARE-STRING-001 | `play_loop.py` AoO concentration check (line ~2524): `world_state.entities.get(turn_ctx.actor_id, {}).get("concentration_bonus", 0)` — uses bare string key instead of `EF.CONCENTRATION_BONUS`. Rule #1 violation. Inconsistent with all other 5 concentration bonus reads in the same file (lines 779, 825, 868, 1409, 2437) which correctly use `EF.CONCENTRATION_BONUS`. Silent if the EF constant ever changes. | WO-AUDIT-SPELL-012 (2026-03-01) | **CLOSED** — Batch AL WO1 CEC (d2351e3). Bare string → EF.CONCENTRATION_BONUS at play_loop.py:2579. |
| FINDING-AUDIT-SPELL-012-REQUIRES-ATTACK-ROLL-DEAD-001 | `SpellDefinition.requires_attack_roll: bool = False` is declared and set on TOUCH/RAY spell definitions, but is **never read** in `SpellResolver.resolve_spell()`. TOUCH/RAY targets are added to affected_entities without executing a touch attack roll. Coverage map correctly marks TOUCH/RAY as PARTIAL; this finding adds specificity: the `requires_attack_roll` field is write-only at runtime. Not a new gap, but the dead field is a trap for future builders assuming the flag gates resolution. | WO-AUDIT-SPELL-012 (2026-03-01) | OPEN |
...
| FINDING-ENGINE-CONDITIONS-LEGACY-FORMAT-001 | `get_condition_modifiers()` safety guard returns zero modifiers for legacy `["condition"]` list format — resolvers calling it will silently miss conditions stored in list format. DR flat-footed required explicit workaround. | Batch AF WO4 | **CLOSED** — WO-ENGINE-CONDITIONS-LEGACY-FIX-001 ACCEPTED (801875d, 2026-03-01). CLF-001..008. ValueError raised on non-dict input; list-format silent-zero path eliminated. |
...
| FINDING-UI-PENDING-ROLL-OUT-OF-SCOPE-001 | `pending_roll` state — emitting `RollResult` from the attack_roll handler covers the post-roll dice slip wire (GAP-09 delivered). Pre-roll state tracking (knowing a roll is about to happen, staging the dice tray visually before the roll) requires the engine to signal "roll imminent" before the d20 fires. No current hook in attack flow for pre-roll notification. Requires design: either (a) new pre-roll event type in ws_protocol, or (b) optimistic UI staging based on attack intent. Out of scope for Stage 4 WO. | WO-UI-PHASE1-POLISH-001 debrief (2026-03-01) | DEFERRED — requires pre-roll design; no immediate correctness gap; Stage 5+ scope |
| FINDING-UI-EF-ABILITY-SCORE-CONSTANTS-MISSING-001 | No `EF.STR`, `EF.DEX`, `EF.CON`, `EF.INT`, `EF.WIS`, `EF.CHA` constants in `aidm/schemas/entity_fields.py`. Only modifier variants exist (EF.STR_MOD, EF.DEX_MOD, etc.). Raw ability scores stored under bare string keys per `aidm/chargen/ability_scores.py:ABILITY_NAMES = ("str", "dex", "con", "int", "wis", "cha")`. ws_bridge uses bare strings for the abilities dict in CharacterState. Rule #1 (use EF.* constants) technically violated but no EF constant exists to use. Either add EF.STR/DEX/CON/INT/WIS/CHA constants pointing to raw score keys, or formally document ABILITY_NAMES as the canonical authority. | WO-UI-PHASE1-POLISH-001 debrief (2026-03-01) | OPEN — LOW. Either add EF.* constants or document ABILITY_NAMES as canonical. Blocks strict Rule #1 compliance for ability score reads. |
| FINDING-ENGINE-SPELLCASTINTENT-FROMDICT-MISSING-001 | `SpellCastIntent` dataclass has no `from_dict()` classmethod. `to_dict()` is now complete (Batch AL WO3 ITD adds spontaneous_inflict + spontaneous_summon). Serialization is one-way: to_dict() works, but no deserialization path exists. WO3 ITD-004 gate test was adapted to avoid testing round-trip. Any future code that persists and reloads SpellCastIntent (replay, undo, session save) will need from_dict() manually written. | Batch AL WO3 ITD debrief (2026-03-01) | DEFERRED — no current consumer of from_dict(); serialization is one-way by intent; add from_dict() when a deserialize consumer is added |
