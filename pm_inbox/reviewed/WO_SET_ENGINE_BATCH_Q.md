...
**Issued by:** Slate (PM)
**Date:** 2026-02-27 (updated 2026-02-27 session 10)
**Lifecycle:** ARCHIVE — BATCH Q COMPLETE
**To:** Chisel (lead builder)
**Batch:** Q — 4 WOs, 32 gate tests (32/32 landed)
**Prerequisite:** ENGINE BATCH P ACCEPTED

**Note:** WO detail is inline in this file. Combined format (inbox at cap).

**Status:**
- WO1 — ENGINE-GREAT-CLEAVE — **ACCEPTED** 9/9 SAI (commit `1a0b128`)
- WO2 — ENGINE-IMPROVED-UNCANNY-DODGE — **ACCEPTED** 8/8 new (commit `5b41a2f`)
- WO3 — ENGINE-WEAPON-FOCUS — **ACCEPTED** 8/8 SAI (commit `d0e4ef2`)
- WO4 — ENGINE-WEAPON-SPECIALIZATION — **ACCEPTED** 8/8 SAI (commit `056e525`)

**Pre-existing failures:** 141 (as of Batch T close).
...
**PHB/SRD citation(s):** PHB p.102 — "You gain a +1 bonus on all attack rolls you make using the selected weapon."
**RAW_FIDELITY_AUDIT.md status:** NEW ROW — Section 11 (Combat Feats — Attack Modifiers). Builder adds row on delivery.
**CP decision reference:** None — no prior decision. Weapon type = categorical key (light/one-handed/two-handed/ranged/natural) is a simplification from PHB's per-specific-weapon design.
**Deviation status:** FULL (categorical key is a documented simplification, not a deviation from bonus mechanics)
**Known ambiguity:** No — +1 attack bonus is unambiguous.

### Parallel Implementation Paths

Per Resolver Parity Map (BUILDER_FIELD_MANUAL.md #34):
- **`attack_resolver.py` — resolve_attack()** — PRIMARY. Must add +1 attack bonus here.
- **`full_attack_resolver.py` — resolve_single_attack_with_critical()** — INDEPENDENT COPY. Must add +1 attack bonus here separately. (**CRITICAL**: this is the 21-modifier drift path from FINDING-AUDIT-FULL-ATTACK-MODIFIER-DRIFT-001 — do NOT skip this file.)
- **`natural_attack_resolver.py`** — delegates to `resolve_attack()` (confirmed Batch T WO1). Inherits automatically — no separate touch needed. Verify on boot.

Builder: confirm parity across all three paths in debrief Pass 1.
...
**PHB/SRD citation(s):** PHB p.102 — "You deal an extra 2 points of damage when using the selected weapon." Damage bonus is pre-crit and multiplied by crit multiplier per PHB p.224 ("Roll the damage with all your usual bonuses one extra time").
**RAW_FIDELITY_AUDIT.md status:** NEW ROW — Section 11 (Combat Feats — Attack Modifiers). Builder adds row on delivery.
**CP decision reference:** None — no prior decision. Same categorical weapon_type key as Weapon Focus.
**Deviation status:** FULL
**Known ambiguity:** No — +2 damage is unambiguous. Pre-crit is standard per PHB damage rules.

### Parallel Implementation Paths

Per Resolver Parity Map (BUILDER_FIELD_MANUAL.md #34):
- **`attack_resolver.py` — resolve_attack()** — PRIMARY. Add +2 damage bonus at damage computation site (after enhancement_bonus, before crit multiplication).
- **`full_attack_resolver.py` — resolve_single_attack_with_critical()`** — INDEPENDENT COPY. Check whether the damage path routes through a shared function or is independent. If independent, add +2 here too. Document in debrief.
- **`natural_attack_resolver.py`** — delegates to `resolve_attack()`. Inherits automatically. Verify on boot.

Builder: confirm parity across all paths in debrief Pass 1.
...
- [x] WO1 committed — GC: 9/9 PASS (SAI confirmed, `1a0b128`)
- [x] WO2 committed — IUD: 8/8 PASS (new implementation, `5b41a2f`)
- [x] WO3 committed — WFC: 8/8 PASS (SAI confirmed, `d0e4ef2`)
- [x] WO4 committed — WSP: 8/8 PASS (SAI confirmed, `056e525`)
- [x] RAW_FIDELITY_AUDIT.md — Weapon Focus + Weapon Specialization rows added (Section 11)
- [x] Zero regressions on full suite (141 pre-existing failures, 0 new)
