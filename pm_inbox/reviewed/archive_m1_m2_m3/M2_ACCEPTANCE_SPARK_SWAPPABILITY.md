...
|-------|---------|----------------|---------|
| **Qwen3 8B (4-bit)** | Primary SPARK | 8192 | 5-6 GB |
| **Qwen3 4B (4-bit)** | Fallback SPARK | 8192 | 3-4 GB |
...
**Test Procedure:**
1. Load default config (default: "qwen3-8b-instruct-4bit")
2. Verify SPARK loaded is Qwen3 8B
3. Override via environment variable: `AIDM_SPARK_MODEL=qwen3-4b-instruct-4bit`
4. Reload AIDM
5. Verify SPARK loaded is Qwen3 4B

**Pass Criteria:**
- ✅ Default SPARK matches config (Qwen3 8B)
- ✅ Override via env var succeeds (Qwen3 4B loaded)
...
```python
# Test code
assert spark.model_id == "qwen3-8b-instruct-4bit"  # Default

os.environ["AIDM_SPARK_MODEL"] = "qwen3-4b-instruct-4bit"
reload_aidm()

assert spark.model_id == "qwen3-4b-instruct-4bit"  # Override
```
...
**Test Procedure:**
1. Run canonical scenario with Qwen3 8B SPARK
2. Capture BOX output (attack result, damage)
3. Swap to Qwen3 4B SPARK (reload config)
...
**Verification Method:**
```python
# Run 1: Qwen3 8B SPARK
result_qwen8b = run_scenario(spark="qwen3-8b-instruct-4bit", seed=42)

# Run 2: Qwen3 4B SPARK
result_qwen4b = run_scenario(spark="qwen3-4b-instruct-4bit", seed=42)

# Verify BOX determinism
assert result_qwen8b.attack_result == result_qwen4b.attack_result  # IDENTICAL
assert result_qwen8b.damage == result_qwen4b.damage  # IDENTICAL
assert result_qwen8b.hit == result_qwen4b.hit  # IDENTICAL

# SPARK narrations MAY differ (acceptable)
# result_qwen8b.narration != result_qwen4b.narration  # ALLOWED
```
...
**Test Procedure:**
1. Load SPARK with `json_mode: false` (e.g., Qwen3 8B via llama.cpp)
...
spark = load_spark("qwen3-8b-instruct-4bit")
...
2. Attempt to load Qwen3 8B (requires 6 GB RAM)
3. Verify system falls back to Qwen3 4B (requires 4 GB RAM)
4. Verify user notification displayed

**Pass Criteria:**
- ✅ OOM detected (exception caught)
- ✅ Fallback to Qwen3 4B succeeds
...
```python
# Mock OOM condition
with mock_oom_on_load("qwen3-8b-instruct-4bit"):
    spark = load_spark_with_fallback(default="qwen3-8b-instruct-4bit", fallback="qwen3-4b-instruct-4bit")

# Verify fallback succeeded
assert spark.model_id == "qwen3-4b-instruct-4bit"
...
**Fail Criteria:**
- ❌ Hard-coded model path detected (e.g., `"models/qwen3-8b.gguf"` in source)
...
grep -r '"qwen3\|"gemma3\|"llama' aidm/ --include="*.py" | grep -v "# comment"
...
**Test Procedure:**
1. Swap SPARK to different model (Qwen3 8B → Qwen3 4B)
...
response_qwen8b = generate_narration(spark="qwen3-8b-instruct-4bit", event=attack_event)
assert "[NARRATIVE]" in response_qwen8b  # LENS provenance

# Generate with Qwen3 4B
response_qwen4b = generate_narration(spark="qwen3-4b-instruct-4bit", event=attack_event)
assert "[NARRATIVE]" in response_qwen4b  # LENS provenance

# Verify BOX mechanics unchanged
assert extract_damage(response_qwen8b) == None  # No mechanical claim from SPARK
assert extract_damage(response_qwen4b) == None  # No mechanical claim from SPARK
```
