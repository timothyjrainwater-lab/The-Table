...
**Last updated:** 2026-02-27 (session 3) — **ENGINE BATCH L ACCEPTED** (Dispatch #21, commit ba3e62f, 32/32, 0 regressions): Improved Disarm + Improved Grapple + Improved Bull Rush + Spell Penetration. New findings: FINDING-ENGINE-IMPROVED-DISARM-COUNTER-001, FINDING-ENGINE-IMPROVED-DISARM-BONUS-001, FINDING-ENGINE-IMPROVED-GRAPPLE-BONUS-001, FINDING-ENGINE-IMPROVED-BULL-RUSH-BONUS-001, FINDING-ENGINE-IMPROVED-OVERRUN-AOO-001 (all LOW, future WOs). **ENGINE BATCH J ACCEPTED** (Dispatch #19, commit 03a2a47, 32/32): IA + SH + SS + RN. **ENGINE BATCH K ACCEPTED** (Dispatch #20, commit 4cb2f72, 34/34): CF + DH + CS + IC. **OSS DATA BATCH A ACCEPTED** (commit 0b73fb9): LST-PARSER + Feats + Equipment + Spells + Class Tables. **ENGINE BATCH M READY** (all 4 WO files live in inbox): Cover fix + Encumbrance + SA auto-immune + Weapon proficiency (34 tests). Dispatch when ready. **Note:** play_loop.py AoO suppression (WOs 1-3) landed in Batch J commit (03a2a47) — Batch L commit (ba3e62f) contains test files + save_resolver.py. **Inbox hygiene:** Thunder to call on: TUNING_001 (2), MEMO_TTS, PROBE (3), BURST_INTAKE_QUEUE, PREFLIGHT_CANARY_LOG.
...
| FINDING-SAI-FRAGMENTATION-001 | LOW | OPEN | `sneak_attack.py` uses three different immunity field names: `immune_to_critical_hits`, `immune_to_sneak_attack`, `EF.CRIT_IMMUNE`. Fragmentation risk — new immunity types may be added to wrong field. Future normalization WO recommended. Non-blocking. **Partial progress:** `EF.CREATURE_TYPE` landed as first-class EF constant via WO-ENGINE-FAVORED-ENEMY-001. When normalization WO runs, sneak_attack.py creature_type check should read from `EF.CREATURE_TYPE` rather than bare string field. |
...
| FINDING-ENGINE-ENCUMBRANCE-CATALOG-001 | LOW | OPEN | Item catalog not threaded into resolver hot paths — two manifestations: (1) heavy load should suppress Monk WIS AC (PHB p.41, attack_resolver); (2) heavy load should block Barbarian Fast Movement (PHB p.26, movement_resolver). Single catalog-plumbing WO closes both simultaneously. Supersedes FINDING-ENGINE-MONK-ENCUMBRANCE-001 + FINDING-ENGINE-FASTMOVEMENT-LOAD-001. |
| GAP-B | HIGH | OPEN | llama-cpp-python blocks Qwen3/Gemma3 (needs VS Build Tools) |
| FINDING-ENGINE-COVER-VALUES-001 | HIGH | OPEN | Cover bonuses wrong vs PHB p.150-151. Engine `CoverType`: STANDARD +4 AC/+2 Ref (correct for half), IMPROVED +8 AC/+4 Ref (wrong — no PHB basis for +8; three-quarters should be +4 AC/+3 Ref). Also: orphaned `HALF_COVER`/`THREE_QUARTERS_COVER` enum in `terrain.py:34-35` unused. WO needed: rename STANDARD→HALF/IMPROVED→THREE_QUARTERS, fix three-quarters to +4 AC/+3 Ref, remove bogus +8 tier, wire unused enum. Confirmed via direct read of `terrain_resolver.py:242-253`. |
| FINDING-ENGINE-IMPROVED-DISARM-COUNTER-001 | LOW | OPEN | Counter-disarm bonus from Improved Disarm (opponent's failed disarm attempt should not disarm attacker with feat). Deferred from Batch L. Future WO. |
...
| WO-ENGINE-RUN-ACTION-001 | **ACCEPTED** — 8/8 Gate ENGINE-RUN-ACTION (RN-001–RN-008). Batch J, commit 03a2a47. Run action: 4× speed (5× with Run feat), −1 initiative next round, no AoO avoidance while running. | 03a2a47 |
| WO-ENGINE-STAGGERED-ACTION-ECONOMY-001 | **ACCEPTED** — 8/8 Gate ENGINE-STAGGERED-ACTION-ECONOMY (SA-001–SA-008). Batch I, commit f671cdb. Staggered condition action economy wired in `action_economy.py` + `play_loop.py`. Standard/move allowed (not full-round). | f671cdb |
| WO-ENGINE-CONCENTRATION-VIGOROUS-001 | **ACCEPTED** — 8/8 Gate ENGINE-CONCENTRATION-VIGOROUS (CV-001–CV-008). Batch I, commit f671cdb. Vigorous motion DC threshold for Concentration checks wired in `play_loop.py`. | f671cdb |
| WO-ENGINE-DAZZLED-CONDITION-001 | **ACCEPTED** — 8/8 Gate ENGINE-DAZZLED-CONDITION (DZ-001–DZ-008). Batch I, commit f671cdb. DAZZLED condition: −1 attack penalty + ranged dim-light miss chance. | f671cdb |
