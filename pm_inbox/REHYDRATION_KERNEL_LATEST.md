...
**Mandatory dispatch sections:** Delivery footer, Integration Seams, Assumptions to Validate, **Parallel Implementation Paths**, Preflight, Audio Cue, **Debrief Required** (every dispatch must include the debrief template — builder files to `pm_inbox/reviewed/DEBRIEF_[WO-ID].md` on completion). Optional: Debrief Focus (0-2 from bank).
...
**ML Preflight Checklist (mandatory in every builder debrief Pass 1):** Builder must confirm: (1) ML-001: gap verified to exist, not SAI ghost; (2) ML-003: Assumptions to Validate step completed; (3) ML-004: pre-existing failure count noted + retry cap honored; (4) ML-005: `git status` clean before commit; (5) ML-006: Authority Tag present (RAW or HOUSE_POLICY); (6) Parallel implementation paths identified and parity verified (see `BUILDER_FIELD_MANUAL.md` #34). PM rejects debrief if any item is unaddressed.
**Parallel Implementation Paths (dispatch section, LOCKED — 2026-02-27):** Every WO that touches a resolver function must list ALL code paths that implement the same logical operation. Builder must verify parity across all paths before filing the debrief. The dispatch doc itself must list the known parallel paths in this section. If the WO author (PM) does not know the parallel paths, the section reads "Builder: identify parallel paths as first task." Source: AUDIT-WO-001 found 21 modifiers drifted between resolve_attack() and resolve_full_attack() over 30+ WOs because no dispatch ever flagged full_attack_resolver.py as a second touch site.
...
**Sweep Audit Cadence (LOCKED — 2026-02-27):** Every 5 engine batches, PM files one Anvil-seat audit WO targeting a specific subsystem (attack modifiers, saves, conditions, spellcasting, action economy). Anvil reads code, files findings as FINDING-AUDIT-* entries, never writes production code. PM triages findings into builder WOs. Sonnet for focused single-subsystem sweeps; escalate to Opus for large cross-cutting audits (10+ files, high breadth + depth). Source: AUDIT-WO-001 caught 21-modifier drift that accumulated silently across 30+ WOs — periodic sweeps prevent this class of failure from recurring.
