...
from aidm.schemas.saves import SaveType, SRCheck
...
class SpellDefinition:
    ...
    """WO-ENGINE-SR-SPELL-PATH-001: Whether this spell allows spell resistance.
    PHB p.177: Most spells allow SR. Some (e.g., conjuration healing, some divinations)
    do not. When False, SR check is skipped entirely for this spell."""
...
class SpellResolution:
    ...
    sr_events: Tuple[Event, ...] = field(default_factory=tuple)
    """WO-ENGINE-SR-SPELL-PATH-001: SR check events emitted during resolution."""
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.sr_events, tuple):
            object.__setattr__(self, 'sr_events', tuple(self.sr_events))
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sr_events": [e.to_dict() for e in self.sr_events],
            "error": self.error,
        }
...
class SpellResolver:
    ...
    def resolve_spell(
        ...
        # WO-ENGINE-SR-SPELL-PATH-001: SR events collected across all targets
        sr_events_list: list = []
        ...
        for entity_id in affected_entities:
            ...
            # WO-ENGINE-SR-SPELL-PATH-001: SR check gate (PHB p.177)
            ...
            if spell.spell_resistance and target.spell_resistance > 0:
                if world_state is not None:
                    sr_check = SRCheck(
                        ...
                        source_id=caster.caster_id,
                    )
                    sr_passed, sr_evts = check_spell_resistance(
                        sr_check=sr_check,
                    ...
                    sr_events_list.extend(sr_evts)
                    if not sr_passed:
                        continue  # SR blocks — skip this target entirely
        ...
        return SpellResolution(
            ...
            stps=tuple(stps),
            sr_events=tuple(sr_events_list),
        )
