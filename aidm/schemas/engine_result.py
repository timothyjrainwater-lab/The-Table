...
BOUNDARY LAW (BL-007): EngineResult freezes on creation via __post_init__.
Any attempt to modify fields after construction raises EngineResultFrozenError.
This is enforced by test_boundary_law.py BL-007.

WHY: EngineResult is the audit record of what BOX decided. If it can be
modified after creation, the narration layer or any downstream consumer
could silently corrupt the authoritative outcome.
...
class EngineResult:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "EngineResult":
        ...
        All required fields (result_id, resolved_at) must be present
        in the data — no fallback generation (BL-017, BL-018).
        """
        # Parse datetime — must be present
        ...
        # Parse status enum
        status = EngineResultStatus(data["status"])

        # Temporarily disable freeze to set fields
        result = object.__new__(cls)
        object.__setattr__(result, "_frozen", False)
        object.__setattr__(result, "result_id", data["result_id"])
...
class EngineResultBuilder:
    ...
    def build(
        ...
        Args:
            result_id: Unique identifier for this result (BL-017: must be injected,
                       may be generated by caller e.g. str(uuid.uuid4()))
            resolved_at: Resolution timestamp (BL-018: must be injected,
                         may be generated by caller e.g. datetime.utcnow())
    ...
    def build_failure(self, reason: str, result_id: str = "", resolved_at: Optional[datetime] = None) -> EngineResult:
        """Build a failed EngineResult.

        Convenience method for failure cases.
        result_id and resolved_at must be provided by caller.
        """
        return self.build(result_id=result_id, resolved_at=resolved_at, status=EngineResultStatus.FAILURE, failure_reason=reason)

    def build_aborted(self, reason: str, result_id: str = "", resolved_at: Optional[datetime] = None) -> EngineResult:
        """Build an aborted EngineResult.

        Convenience method for aborted resolutions (e.g., AoO defeated actor).
        result_id and resolved_at must be provided by caller.
        """
        return self.build(result_id=result_id, resolved_at=resolved_at, status=EngineResultStatus.ABORTED, failure_reason=reason)
