...
- **SessionBootstrap**: Campaign load sequence, WorldState reconstruction via replay-first policy
- **Bootstrap features**: Partial write recovery (discard incomplete turns), log sync checks (EventLog turn_end == SessionLog resolved), fail-fast on corruption
- **10× replay verification**: Resume guarantees determinism through 10× replay hash checks
- **Runtime Session**: RuntimeSession manager for intent → result → log flow (per IPC_CONTRACT.md)
- **Runtime Experience Design (M1_5_RUNTIME_EXPERIENCE.md)**: Text-only solo UX specification
  - Resume flow: Campaign load → replay → ready prompt (with partial write recovery)
  - Session start: Text display of game state, initiative order, PC status
  - Turn loop: Input → clarify → confirm → resolve → display → next turn
  - Exit semantics: Graceful exit, emergency exit (Ctrl+C), auto-save policy
  - Engine/presentation boundary: Clear interface contract, no shared mutable state
  - Concrete walkthrough: Full session transcript example (3 turns, goblin encounter)
  - **Implementation NOT started** — design-only work order
...
- **CampaignDescriptor**: Input format for prep pipeline (campaign metadata, story context, expected asset counts)
- **ModelLoadConfig**: Single model configuration (model_type, model_id, device, offload settings)
- **PrepPipelineConfig**: Full pipeline configuration (descriptor, output_dir, model_sequence, stub_mode)
- **GeneratedAsset**: Asset record (asset_id, type, semantic_key, file_path, content_hash, generation_method)
- **PrepAssetManifest**: Campaign asset manifest with sorted asset list (deterministic ordering)
- **PrepPipelineResult**: Pipeline execution result (status, manifest, errors, warnings, execution_log)
- **PrepPipeline class**: Sequential model loading orchestrator
  - **Load → Generate → Unload** pattern (one model at a time to minimize memory)
  - Model sequence: LLM → Image Gen → Music Gen → SFX Gen
  - Stub mode for testing without real models (swappable to real inference)
  - Asset storage: campaign_id/asset_type/files directory structure
  - Content hash computation (SHA256) for all assets
  - Manifest persistence (asset_manifest.json with sorted keys)
- **Asset types**: NPCs (JSON), Portraits (PNG), Scenes (PNG), Music (OGG), SFX (OGG)
- **Demo script**: scripts/prep_pipeline_demo.py (runnable end-to-end demonstration)
- **Reference**: pm_inbox/SONNET-B_WO-M3-PREP-01_prep_pipeline_prototype.md
...
- prep_pipeline.py (WO-M3-PREP-01 — sequential model loading, asset generation)
...
The `Vault/` directory is a **structured D&D 3.5e rules reference** extracted from the core rulebooks. It is not AIDM application code. Do not modify it unless explicitly tasked with Vault work.

- **What it contains:** Obsidian-style knowledge graph — skeleton markdown notes for rules, conditions, and subsystems, linked via `[[wikilinks]]`. Raw extracted page text from PHB and DMG in `00-System/Staging/`. Compiler configs in `00-System/Compiler/passA/`.
- **Current state:** Skeleton notes exist but are **unfilled** (content sections say "To be filled from staged pages"). The extraction pipeline ran (1,750 page files exist in Staging), but the compile step that populates the notes from extracted text was never completed.
- **Size:** ~23,750 files, ~20MB. Includes 283 `.bak` backup files (duplicates, safe to delete).
- **When it becomes relevant:** LLM integration phase (M3+), when the AI DM needs to reference actual rules text for narration, rule lookups, or citation. The extracted page text and citation mappings will be the source material for that feature.
- **Do NOT:** Delete the Vault, reorganize it, or attempt to "finish" the compile pipeline without a specific work order.
...
- **Scope**: Sequential model loading prototype for campaign asset generation
- **Test count impact**: 1744 → 1777 (33 new tests: 17 prep_pipeline + 16 other additions)
- **Runtime**: ~8 seconds (slight increase from asset I/O)

**Implementation:**
- **PrepPipeline class** (aidm/core/prep_pipeline.py, 549 lines):
  - Sequential model loading orchestrator (load → generate → unload pattern)
  - Model sequence: LLM → Image Gen → Music Gen → SFX Gen
  - Each model unloaded before next loads (minimize memory usage)
  - Stub mode for testing without real models (swappable implementation)
  - Asset storage with deterministic paths (campaign_id/asset_type/files)
  - SHA256 content hashing for all generated assets
  - Manifest generation (asset_manifest.json) with sorted keys

- **Schemas** (aidm/schemas/prep_pipeline.py, 394 lines):
  - CampaignDescriptor: Input format (campaign metadata + story context)
  - ModelLoadConfig: Single model configuration (type, id, device, offload)
  - PrepPipelineConfig: Full pipeline config (descriptor, output_dir, model_sequence)
  - GeneratedAsset: Asset record (id, type, path, hash, generation_method)
  - PrepAssetManifest: Sorted asset collection (deterministic ordering)
  - PrepPipelineResult: Execution result (status, manifest, errors, execution_log)

- **Asset Generation** (stub mode):
  - LLM assets: NPCs with JSON format (name, description, traits, dialogue)
  - Image assets: Portraits (1 per NPC) + scenes (PNG format, minimal header)
  - Music assets: Tracks based on mood_tags (OGG format, 2-minute loops)
  - SFX assets: Basic set (sword_clash, door_open, footsteps, ambient_wind)

- **Demo script** (scripts/prep_pipeline_demo.py, 219 lines):
  - Runnable end-to-end demonstration
  - Sample campaign: "The Crimson Tower" (dark fantasy, 3 NPCs, 2 scenes)
  - Generates 15 total assets (3 NPCs + 3 portraits + 2 scenes + 3 music + 4 SFX)
  - Output: artifacts/demo_campaign_001/ with full directory structure

**Test Coverage** (tests/test_prep_pipeline.py, 17 tests):
- **Schema validation** (7 tests): Descriptor, config, asset, manifest serialization
- **Pipeline execution** (8 tests): Basic execution, asset counts, directory structure, content, hashes, error handling, logging, determinism
- **Edge cases** (2 tests): Empty mood_tags fallback, partial model sequence
...
**Architecture Notes:**
- Sequential loading ensures **only one model in memory at a time**
- Stub/real mode is **swappable** (architecture supports real inference)
- Asset IDs follow format: `campaign_id:asset_type:identifier`
- Directory structure: campaign_id/{NPCs,Portraits,Scenes,Music,SFX}/files
- BL-018 compliant (timestamps injected via manifest.generated_at)

**Integration:**
- Builds on PrepOrchestrator (M2) job orchestration patterns
- Complements AssetStore (M2) for asset management
- Feeds M3 Immersion Layer (generated assets ready for consumption)
- No LLM at runtime (all generation during offline prep)

**Next Steps:**
- Replace stub implementations with real model inference
- Integrate ModelSelector for hardware-aware model selection
- Wire generated assets to M3 immersion layer (NpcCard, ImageResult, AudioTrack)
...
- config/REUSE_DECISION.json (source reuse decisions)
- EXTRACTED_SOURCES_QUICK_REF.md (extracted source quick reference)
- ~~WORKSPACE_MANIFEST.md~~ (deleted — severely stale)
- ~~PROJECT_HEALTH_AUDIT.md~~ (deleted — superseded by global audit)
- docs/SOURCE_LAYER_PLAN.md (historical migration plan, moved from root)
- docs/AUDIT_REPORT.md (historical Vault ingestion audit, moved from root)
- ~~docs/AIDM_PROJECT_MASTER_PLAN.md~~ (deleted — superseded by Execution Roadmap V3)
