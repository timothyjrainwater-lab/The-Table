...
**Technical Needs:**
1. **Graceful Degradation:** System must function on minimum spec (Phi-2 2.7B) AND high-end spec (Mistral 7B+)
...
**Example:**
- **SPARK = Phi-2:** Generates "You hit the orc for 10 damage" (hallucinated damage value)
- **BOX:** Ignores SPARK damage claim, computes actual damage (1d8+3 = 7), outputs `[BOX] 7 damage`
- **SPARK = Mistral 7B:** Generates "Your blade strikes true" (no hallucinated damage)
...
- ✅ Default SPARK MUST be specified in configuration (e.g., `default_model: mistral-7b-4bit`)
...
**MUST NOT:**
- ❌ SPARK provider MUST NOT be hard-coded in source (e.g., `model = "mistral-7b"` in Python)
...
def load_llm():
    return LlamaCpp(model_path="models/mistral-7b.gguf")  # STOP CONDITION
```
...
spark:
  - id: "mistral-7b-4bit"
    capabilities:
      context_window: 8192
      json_mode: false  # Mistral v0.2 lacks native JSON mode
      streaming: true
      tool_calling: false
    constraints:
...
grep -r "mistral\|llama\|phi-2" aidm/ --include="*.py" | grep -v "# comment\|\"\"\".*\"\"\""
...
**M2 Transition:**
1. **Create models.yaml** with default SPARK (Mistral 7B 4-bit)
