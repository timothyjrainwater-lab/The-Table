...
| Nonlethal attack (resolve_nonlethal_attack) | PHB p.146 | **IMPLEMENTED** | `attack_resolver.py` | -4 attack penalty; WF + ImprCrit shadow path removed Batch AQ — both now delegate to _compute_finesse_delta/_compute_effective_crit_range shared helpers; get_condition_modifiers() wired for attacker + target. NSP-001..008 (shadow path gate). NSP-001..008 (Batch AW: ghost target — condition modifiers already wired). |
...
| Disarm — two-handed weapon advantage | PHB p.155 | **IMPLEMENTED** | `maneuver_resolver.py` | +4 to attacker_modifier for two-handed weapon on disarm (PHB p.155). Also -4 for light weapon. Both sides apply weapon type modifiers. maneuver_resolver.py:1526-1534. WO-ENGINE-DISARM-2H-BONUS-001 (Batch BC, ghost — pre-existed B-AMB-04). |
...
| SR event emission to EventLog | PHB p.172 | **IMPLEMENTED** | `play_loop.py` | `resolution.sr_events` emitted after spell_cast event in `_resolve_spell_cast()`. `spell_resistance_checked` events stamped with real event IDs. SRE-001..008. WO-ENGINE-SR-EVENTS-EMIT-001. Batch AY. |
...
| Concentration — entangled | PHB p.175 | **IMPLEMENTED** | `play_loop.py` | DC 15 + spell_level for entangled caster; inline elif branch after grappled check. play_loop.py:821-823. WO-ENGINE-CONCENTRATION-ENTANGLED-001 (Batch BC, ghost — pre-existed WO-ENGINE-CONCENTRATION-GRAPPLE-001). |
...
| 17 | ~~**Concentration checks (grappled / entangled / other causes)**~~ | Spellcasting | ~~Only damage-during-casting is checked.~~ **IMPLEMENTED**: Grappled DC=20+spell_level (play_loop.py:818-819), Entangled DC=15+spell_level (play_loop.py:821-823). Unstable surface/other causes still NOT STARTED. | **GRAPPLED+ENTANGLED DONE** — Batch BC |
